...
//uint8_t test = 0x51;
//HAL_SPI_Transmit(&hspi3, (uint8_t*)&test, 1, HAL_MAX_DELAY);
 vConfig();
 vGoStandby();
}
...
bool loraClass::bSendMessage(byte msg[], byte length)
{
 ...
 //	vSpiBurstWrite((RegFifoTxBaseAddr<<8)+tmp, msg, length);
...
/*byte loraClass::bSpiTransfer(byte dat) {
	byte SPDR = dat;
...
void loraClass::vSpiWrite(word dat) {
/*	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET);
	uint8_t test = 0x51;
	HAL_SPI_Transmit(&hspi3, (uint8_t*)&test, 1, HAL_MAX_DELAY);*/

	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);
	uint8_t temp = (dat>>8) | 0x80;
	HAL_SPI_Transmit(&hspi3, (uint8_t*)&temp, 1, HAL_MAX_DELAY);
	while (HAL_SPI_GetState(&hspi3) != HAL_SPI_STATE_READY)
										;
//	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET);
//	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);
	HAL_SPI_Transmit(&hspi3, (uint8_t*)&dat, 1, HAL_MAX_DELAY);
	while (HAL_SPI_GetState(&hspi3) != HAL_SPI_STATE_READY)
										;
...
byte loraClass::bSpiReadByte() {
	byte txByte = 0x00;
	byte rxByte = 0x00;

	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);
	HAL_SPI_TransmitReceive(&hspi3, &txByte, &rxByte, 1, HAL_MAX_DELAY);
	while (HAL_SPI_GetState(&hspi3) != HAL_SPI_STATE_READY)
										;
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET);
	return rxByte;
...
//Concern
byte loraClass::bSpiRead(byte addr) {
...
//	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET);
	...
	byte tmp;
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);
	HAL_SPI_Transmit(&hspi3, (uint8_t*)&addr, 1, HAL_MAX_DELAY);
	while (HAL_SPI_GetState(&hspi3) != HAL_SPI_STATE_READY)
										;
	tmp = bSpiReadByte();
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET);
...
//	tmp = HAL_SPI_Transmit(&hspi3, 0xFF, 1, 100);
//	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET);
	return tmp;
}
	
void loraClass::vSpiBurstWrite(byte addr, byte ptr[], byte length){
	byte i;

//	HAL_SPI_Transmit(&hspi3, ((uint8_t *)&(addr | 0x80)) /*| (uint8_t)0x80*///, 1, 100);
//	HAL_SPI_Transmit(&hspi3, (uint8_t *)&ptr, length, 100);
//	uint8_t temp = addr | 0x80;
//	HAL_SPI_Transmit(&hspi3, ((uint8_t *)(&temp)), 1, 100);
//	for(i=0; i<length; i++) {
//		HAL_SPI_Transmit(&hspi3, (uint8_t *)&ptr[i], length, 100);
//	bSpiTransfer(ptr[i]);
//	}
...
void loraClass::vSpiBurstRead(byte addr, byte ptr[], byte length)
{
/*	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);
	HAL_SPI_Transmit(&hspi3, (uint8_t *)&addr, 1, 100);
	HAL_SPI_Receive(&hspi3, (uint8_t *)ptr, length, 100);
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET);*/
	...
	/*	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);
		...
		while (HAL_SPI_GetState(&hspi3) != HAL_SPI_STATE_READY)
 		...
 		for(i=0; i<length; i++) {
 			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);
 			ptr[i] = bSpiRead(addr);
 		//	ptr[i] = HAL_SPI_Receive(&hspi3, (uint8_t *)ptr, length, 100);
